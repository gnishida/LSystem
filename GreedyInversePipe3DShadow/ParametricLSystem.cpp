...
void ParametricLSystem::draw(const String& model, std::vector<Vertex>& vertices) {
	...
	for (int i = 0; i < model.length(); ++i) {
		if (model[i].c == '[') {
			...
			if (model[i].param_defined) {
				...
				glutils::drawCylinder(glm::vec3(0, 0, 0), length, 1, glm::vec3(1, 1, 1), modelMat, vertices);
	...
	// 背景の壁
	modelMat = glm::mat4();
	modelMat = glm::translate(modelMat, glm::vec3(0, 100, 0));
	modelMat = glm::rotate(modelMat, deg2rad(90), glm::vec3(1, 0, 0));
	glutils::drawQuad(glm::vec3(0, 0, 0), 160, glm::vec3(1, 1, 1), modelMat, vertices);
}
